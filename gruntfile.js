...
gruntfile_cfg.clean = {
  all: ['<%= distdir %>/../*'],
  app: ['<%= distdir %>/*', '!<%= distdir %>/../chainid*', '!<%= distdir %>/../docker*'],
...
function shell_buildBinary(p, a) {
  var binfile = 'dist/chainid-'+p+'-'+a;
  return [
    'if [ -f '+(( p === 'windows' ) ? binfile+'.exe' : binfile)+' ]; then',
      'echo "Chain Platform binary exists";',
    'else',
...
function shell_run(arch) {
  return [
    'docker rm -f chainid',
    'docker run -d -p 9000:9000 -v $(pwd)/dist:/app -v /tmp/chainid:/data -v /var/run/docker.sock:/var/run/docker.sock:z --name chainid chainid/base /app/chainid-linux-' + arch + ' --no-analytics'
  ].join(';');
}
